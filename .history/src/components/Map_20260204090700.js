    ...
    try {
      ...
      map.current.on('click', 'districts-fill', (e) => {
        if (e.features.length > 0) {
          ...
          const RESERVED_KEYS = ['id', 'name', 'type', 'image', 'description', 'population', 'area', 'color', 'gradientStart', 'gradientEnd', 'gradientDirection'];
          const customEntries = Object.entries(district || {}).filter(([k]) => !RESERVED_KEYS.includes(k));
          ...
          const fieldsHTML = customEntries.length > 0
            ? customEntries.map(([key, value]) => `
              <div class="popup-card__row">
                <span class="popup-card__label">${escape(formatLabel(key))}</span>
